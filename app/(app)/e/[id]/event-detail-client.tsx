...
import { ConflictDialog } from "@/components/events/conflict-dialog";
...
export function EventDetailClient({
  ...
  const isConfirmed = userRegs?.some((r) => r.event_id === id && r.status === "confirmed") ?? false;
  // isGoing means confirmed only (waitlisted attendees are not yet "going")
  const isGoing = isConfirmed;
  ...
  return (
    <div className="dot-grid min-h-[80vh]">
      ...
      {/* Conflict dialog */}
      {showConflictDialog && conflictingEvent && (
        <ConflictDialog
          conflictingEventTitle={conflictingEvent.event.title}
          onConfirm={() => { rsvp(id); setShowConflictDialog(false); }}
      ...
      <div className="mx-auto grid max-w-7xl gap-stack-lg px-grid-margin py-stack-xl md:grid-cols-12">
        ...
        <div className="space-y-stack-lg md:col-span-8">
          ...
          {/* Updates */}
          <UpdatesSection eventId={id} isHost={isHost} />
        </div>
        ...
        <div className="md:col-span-4">
          <div className="sticky top-28 space-y-stack-md border-4 border-on-background bg-surface p-stack-lg hard-shadow">
            ...
            {canRegister && !session && (
              <a
                href="/login"
                className="block min-h-[44px] w-full border-2 border-on-background bg-primary px-6 py-3 text-center font-mono text-label-mono uppercase font-semibold text-on-primary hard-shadow hard-shadow-hover transition-transform"
              >
                LOG IN TO RSVP
              </a>
            )}
            ...
            {/* Share button — upcoming/live events */}
            {(uiState === "upcoming" || uiState === "live") ? (
              <button
                type="button"
                onClick={() => {
